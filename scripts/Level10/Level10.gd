...
const scnDuck  = preload("res://scenes/Levels/Level10/Duck10.tscn")
const scnHalfFruit  = preload("res://scenes/halfFruit.tscn")
const scnBoss  = preload("res://scenes/Levels/Level10/Boss10.tscn")
...
func _process(delta):
	if get_child(3).position.x <= -28 and get_node("/root/Main").ducksInField <= 0:
		#get_tree().get_root().get_child(0).Level +=1
